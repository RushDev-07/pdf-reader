...
for i in range(1, 12):  # Adjust range as needed
    try:
        # Find the button with specific data-qstr and title
        button = driver.find_element(
            By.XPATH,
            f'//a[@data-qstr="12--{i}"][contains(@title, "View Certificate")]'
        )
        ...
        if data_attribute and 'base64,' in data_attribute:
            ...
            # Save the PDF file
            with open(f"SOURCE_DOCUMENTS/certificate_{i}.pdf", "wb") as pdf_file:
                pdf_file.write(pdf_data)
            print(f"Certificate {i} downloaded successfully.")
        ...
        print(f"An error occurred with certificate {i}: {e}")
